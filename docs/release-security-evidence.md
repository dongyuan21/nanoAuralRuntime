...
the six declared package roots and the exact seven-file dist-info contract;
even commonly generated but uncontracted members such as `INSTALLER` are
rejected.  It requires all six `__init__.py` files, source-identical
...
`NOTICE`, `pyproject.toml`, generated `setup.cfg`, and all six package trees.
